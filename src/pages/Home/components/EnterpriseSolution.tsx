...
export default function EnterpriseSolution() {
  return (
    <section id="enterprise-solutions" className="py-24 px-6 relative overflow-hidden bg-transparent">
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="text-center mb-16">
          <motion.h2
            ...
            viewport={{ once: true }}
            className="text-3xl md:text-5xl font-black text-secondary-fixed mb-6 font-display tracking-tight"
          >
            ERP System Software Developed <span className="text-white">Exclusively for Ethiopian Companies</span>
          </motion.h2>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {modules.map((module, i) => (
            <motion.div
              ...
              className="bg-white/5 backdrop-blur-sm p-8 rounded-2xl flex flex-col items-start gap-6 group hover:bg-secondary hover:-translate-y-1 transition-all duration-300 border border-white/5"
            >
              <div className="w-14 h-14 rounded-full bg-secondary-fixed/20 flex items-center justify-center text-secondary-fixed group-hover:bg-white group-hover:text-secondary transition-colors duration-300">
                {module.icon}
              </div>
              <div className="space-y-4">
                <h3 className="text-lg font-black text-white group-hover:text-white font-display leading-tight transition-colors">{module.title}</h3>
                <p className="text-white/50 text-xs leading-relaxed font-medium group-hover:text-white/90 transition-colors">
                  {module.desc}
                </p>
              </div>
            </motion.div>
        ...
        <div className="mt-20 text-center">
          ...
          >
            <Link
              to="/erp-system"
              className="group relative inline-flex items-center gap-3 px-12 py-5 bg-white/10 text-white font-black uppercase tracking-widest text-xs overflow-hidden rounded-sm transition-all duration-500 shadow-xl hover:shadow-[0_0_40px_rgba(30,64,175,0.3)] ring-1 ring-white/10"
            >
              <span className="relative z-10 group-hover:text-white transition-colors duration-300">Read More About ERP</span>
              <div className="absolute inset-0 bg-gradient-to-r from-secondary to-secondary-fixed scale-x-0 group-hover:scale-x-100 origin-left transition-transform duration-500" />
